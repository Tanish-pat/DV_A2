    ...
    try {
        ...
        Papa.parse(csvData, {
            ...
            step: function (row) {
                ...
                const damage_grade = row.data['damage_grade'];

                if (district_id) {
                    if (!damage_grade_per_district[district_id]) {
                        damage_grade_per_district[district_id] = {
                            'Grade 1': 0,
                            'Grade 2': 0,
                            'Grade 3': 0,
                            'Grade 4': 0,
                    ...
                    if (damage_grade === '') {
                        damage_grade_per_district[district_id]['Missing']++;
                    } else if (damage_grade_per_district[district_id][damage_grade] !== undefined) {
                        damage_grade_per_district[district_id][damage_grade]++;
                    }
                }
            },
            complete: function () {
                console.log("Data loading complete");
                console.log("Damage grade per district:", damage_grade_per_district);
...
function createTreeMapData(selectedCategory) {
    ...
    for (const district_id in damage_grade_per_district) {
        const districtData = {
            name: district_id,
            children: []
        };
        ...
        if (selectedCategory === "All") {
            ...
            for (const damage_grade in damage_grade_per_district[district_id]) {
                const value = damage_grade_per_district[district_id][damage_grade];
            ...
            const value = damage_grade_per_district[district_id][selectedCategory];
